...
-  $minDF (float) - ignore tokens that have a samples frequency strictly
...
To transform a collection of text samples use ``transform`` method.
Example:
...
-  WordTokenizer - select tokens of 2 or more alphanumeric characters
   (punctuation is completely ignored and always treated as a token
   separator).
-  NGramTokenizer - continuous sequence of characters of the specified
