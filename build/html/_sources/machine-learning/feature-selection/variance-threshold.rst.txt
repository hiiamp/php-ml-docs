...
``VarianceThreshold`` is a simple baseline approach to feature
...
-  $threshold (float) - features with a variance lower than this
...
``VarianceThreshold`` implements ``Transformer`` interface so it can be
used as part of pipeline:
