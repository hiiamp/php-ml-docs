...
The K-Means algorithm clusters data by trying to separate samples in n
...
To divide the samples into clusters simply use ``cluster`` method. It's
return the ``array`` of clusters with samples inside.
...
K-means++ method selects initial cluster centers for k-mean clustering
in a smart way to speed up convergence. It use the DASV seeding method
consists of finding good initial centroids for the clusters.

random
^^^^^^

Random initialization method chooses completely random centroid. It get
